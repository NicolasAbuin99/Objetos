...
public class TestApiStream {
    public static void main(String[] args) {
        ...
        personas //busca a ana o a juan, el primero que este en la lista
                .stream()
                .filter(p->p.getNombre().equals("Ana") || p.getNombre().equals("Juan"))
                .forEach(System.out::println);

        System.out.println("\n-- select * from personas where nombre like 'ja%'; --");
        personas    //busca registro cuyo nombre empieza con ja
                .stream()
                .filter(p->p.getNombre().toLowerCase().startsWith("ja")) //empieza
                .forEach(System.out::println);

        System.out.println("\n-- select * from personas where nombre like '%a'; --");
        personas //busca registro cuyo nombre termina con a
                .stream()
                .filter(p->p.getNombre().toLowerCase().endsWith("a"))
                .forEach(System.out::println);
//% REPRESENTA CUALQUIER COSA, PUEDE SER UN CARACTER, NUMERO, SIGNO E INCLUSO NADA
        System.out.println("\n-- select * from personas where nombre like '%ar%'; --");
        personas //busca registro cuyo nombre tiene ar en cualquier parte, osea debe contenerlo
                .stream()
